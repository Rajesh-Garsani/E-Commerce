...
class PerPathSessionMiddleware(SessionMiddleware):
    """
    Replace Django's SessionMiddleware and choose session cookie name by request path.

    - Admin (admin_prefix) -> ADMIN_SESSION_COOKIE_NAME (default 'admin_sessionid')
      cookie path = '/admin/'
    - Everything else -> USER_SESSION_COOKIE_NAME (default SESSION_COOKIE_NAME)
      cookie path = '/'
    """

    def __init__(self, get_response=None):
        super().__init__(get_response)
        # Try to auto-detect admin prefix via reverse('admin:index'); fall back to '/admin/'
        try:
    ...
    def process_response(self, request, response):
        ...
        if chosen_name != default_name and default_name in response.cookies:
            ...
            # 🚨 Remove any duplicate admin_sessionid with wrong path
